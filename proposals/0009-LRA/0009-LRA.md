...
that they can be invoked as JAX-RS endpoints. The both annotations are expected
...
or cancelled.
 ...
 - If the nested LRA is closed but the outer LRA is canceled
then the participants registered with the nested LRA will be told to
compensate.
 - If the nested LRA is canceled the the outer LRA can be still closed.
...
The `@Forget` method is a standard REST endpoind expected to be used with
