...
The proposal introduces an API for loosely coupled services to coordinate
...
* `REQUIRED`: a new LRA is started if none is present and ended when the
  method finishes
...
`@Complete` defines a JAX-RS endpoint available with `@POST` where the method
serves as a handler invoked by transaction manager when LRA completes succesfully.
The `@Compensate` defines a JAX-RS endpoid availabe with `@POST` invoked
in case LRA fails

If the bean cannot perform the compensation or completion
activity immediately then it should return a `202 Accepted` status code
together with a status URL in a JAX-RS `Location` header. Calling to the status
URL reports information about current status of the participant as
[discussed later](#reporting-the-status-of-a-compensator).
An alternative to providing a status URL is to provide a JAX-RS method annotated
with `@GET` and `@Status`. Such JAX-RS endpoint is then provided as
the status URL automatically.
...
annotated with a `@Forget` annotation using method `@DELETE`.
...
annotated with `@Compensate` or `@Complete`

* are not able to complete their task synchronously
* and do not return a status URL in the `Location` header
...
a valid status value (in the body response) from the enum `CompensatorStatus`:
...
Example of a method providing a status information

```java
@GET
@Path("/status")
@Status
public Response status(@HeaderParam(LRA_HTTP_HEADER) String lraId) {
    if (activity.status == null) {
        return Response.status(Response.Status.BAD_REQUEST)
          .entity(Entity.text("LRA is not active")).build();
...
How the application obtains an LRAManagement instance is unspecified. The
...
2. Performing a `GET` on `<base uri>/lra-coordinator/<LRAId>/completed`
...
3. Performing a `POST` on `<compensator URL>/compensate` will cause the
...
4. A compensator can be told to complete or compensate (by POSTing to the
...
5. The compensator is assumed to be in the status `Compensated` if it
   ...
   to a post request on `<compensator URL>/complete`.

6. A `500 Internal Server Error` status code returned by
   `<compensator URL>/compensate` or `<compensator URL>/complete` may
   indicate that the compensator is in the state `FailedToCompensate` or
   `FailedToComplete` respectively. The coordinator however is allowed to call
   these endpoints multiple times before deciding that it's not possible to
   complete or compensate. The service writer must take into account that
   complete and compensate endpoints may be called multiple times,
   so the operations must be idempotent.

A compensator can resign from a LRA at any time prior to
